...
Compile with: g++ -std=c++17 -o main_test -g -O2 main_p1.cpp libblst.a -I/opt/homebrew/include/  -L/opt/homebrew/Cellar/openssl@3/3.2.1/lib -lcrypto
...
bool TEST_PIPPENGER_Q_OVER_5_CHES = 0; // q = 2^c (LFG)
bool TEST_PIPPENGER_BGMW95 = 0; // Due to the momory size limitation, some time we need to switch off one of the test.
bool TEST_PIPPENGER_Q_OVER_5_2_c = 1; // q = 2^c (FKSX)
...
digit_decomposition* DIGIT_CONVERSION_HASH_TABLE_2_c;
...
int* BUCKET_SET_2_c;
int* BUCKET_VALUE_TO_ITS_INDEX_2_c;
...
blst_p1_affine* PRECOMPUTATION_POINTS_LIST_3nh_2_c;
blst_p1_affine* PRECOMPUTATION_POINTS_LIST_n_2_c;
...
void init_pippenger_q_over_5_2_c(){
    //Initialize BUCKET_SET and BUCKET_VALUE_TO_ITS_INDEX 
    BUCKET_SET_2_c = new int[B_SIZE_2_c];
    construct_bucket_set_2_c(BUCKET_SET_2_c, q_RADIX);
    std::cout<< "BUCKET_SET constructed. The size of BUCKET_SET is: "<< std::dec << B_SIZE_2_c << std::endl;

    BUCKET_VALUE_TO_ITS_INDEX_2_c = new int[q_RADIX/2 +1];
    for(size_t i = 0; i < B_SIZE_2_c; ++i){
        BUCKET_VALUE_TO_ITS_INDEX_2_c[BUCKET_SET_2_c[i]] = i;
    }
    
    // Initialize  DIGIT_CONVERSION_HASH_TABLE;
    DIGIT_CONVERSION_HASH_TABLE_2_c = new digit_decomposition[q_RADIX+1];
    for(int m: MULTI_SET){
        for (int i = 0; i < B_SIZE_2_c; ++i){
            int b = BUCKET_SET_2_c[i];
            if (m*b <= q_RADIX) DIGIT_CONVERSION_HASH_TABLE_2_c[q_RADIX - m*b] = {m,b,1};
        }
    }
    for(int m: MULTI_SET){
        for (int i = 0; i < B_SIZE_2_c; ++i){
            int b = BUCKET_SET_2_c[i];
            if (m*b <= q_RADIX) DIGIT_CONVERSION_HASH_TABLE_2_c[m*b] = {m,b,0};
        }
    }
    std::cout<< "DIGIT_CONVERSION_HASH_TABLE constructed." <<std::endl;
    
    // ### Initialize the precomputation ###
    PRECOMPUTATION_POINTS_LIST_3nh_2_c = new blst_p1_affine [3*N_POINTS*h_LEN_SCALAR];
    PRECOMPUTATION_POINTS_LIST_n_2_c = new blst_p1_affine [N_POINTS];
    ...
    for(int i = 0; i< N_POINTS; ++i){
        ...
        for(int j = 0; j< h_LEN_SCALAR; ++j){
            for(int m = 1; m <=3; ++m){
            size_t idx_i_j_m =  3*(i*h_LEN_SCALAR +j) + m-1;  
            if(m==1) PRECOMPUTATION_POINTS_LIST_3nh_2_c[idx_i_j_m]  = qjQi;
            // ((m-1)*h_LEN_SCALAR + j)*N_POINTS + i;
            else{PRECOMPUTATION_POINTS_LIST_3nh_2_c[idx_i_j_m] = single_scalar_multiplication(m, qjQi);}
            }
            qjQi = single_scalar_multiplication(q_RADIX, qjQi);
            if(j== h_LEN_SCALAR-1) PRECOMPUTATION_POINTS_LIST_n_2_c[i] = qjQi;  
        }  
    }
    auto ed = std::chrono::steady_clock::now();   
    ...
    std::chrono::microseconds diff = std::chrono::duration_cast<std::chrono::microseconds>(ed -st);
    std::cout<< "PRECOMPUTATION_POINTS_LIST_3nh SUCCESSFULLY CONSTRUCTED" << std::endl;
    std::cout<< "PRECOMPUTATION_POINTS_LIST_n SUCCESSFULLY CONSTRUCTED" << std::endl;
    std::cout << "PRECOMPUTATION Wall clock time elapse is: " << diff.count() << " us "<< std::endl;
}

void free_init_pippenger_q_over_5_2_c(){

    delete[] PRECOMPUTATION_POINTS_LIST_3nh_2_c;
    delete[] PRECOMPUTATION_POINTS_LIST_n_2_c;
    delete[] DIGIT_CONVERSION_HASH_TABLE_2_c;
    delete[] BUCKET_VALUE_TO_ITS_INDEX_2_c;
    delete[] BUCKET_SET_2_c;
}
...
blst_p1_affine pippenger_variant_q_over_5_2_c(uint256_t scalars_array[]){
    
    std::array<std::array< int, 2>, h_LEN_SCALAR+1> ret_MB_expr;
    ...
    blst_p1 acc = {0,1,0}; // acc = INFINITY;

    for(int i = 0; i< N_POINTS; ++i){

        blst_p1 xyzQ;
        trans_uint256_t_to_MB_radixq_expr_2_c(ret_MB_expr, scalars_array[i]);

        if(ret_MB_expr[h_LEN_SCALAR][0] == 1 && ret_MB_expr[h_LEN_SCALAR][1] == 1) {
            blst_p1_from_affine(&xyzQ, &PRECOMPUTATION_POINTS_LIST_n_2_c[i]);
            blst_p1_add_or_double(&acc, &acc, &xyzQ);
        }
        ...
        for(int j = 0; j< h_LEN_SCALAR; ++j){
            ...
            if (m> 0) {
                size_t idx_i_j_m =  3*idx + m-1; 
                points_ptr[idx] = PRECOMPUTATION_POINTS_LIST_3nh_2_c + idx_i_j_m;
                booth_signs[idx] = 0; 
            }
            else{
                size_t idx_i_j_m =  3*idx -m - 1; 
    ...
    buckets = new blst_p1xyzz [B_SIZE_2_c];

    blst_p1_tile_pippenger_d_CHES(&ret, points_ptr, npoints, scalars, booth_signs,\
                                        buckets, BUCKET_SET_2_c, BUCKET_VALUE_TO_ITS_INDEX_2_c, B_SIZE_2_c, d_MAX_DIFF);
    delete[] buckets;
    delete[] points_ptr;
    delete[] booth_signs;    
    delete[] scalars;

    blst_p1_add_or_double(&ret, &ret, &acc);
    blst_p1_affine res_affine;
    blst_p1_to_affine(&res_affine, &ret);
...
void test_pippengers(){
    ...
    std::chrono::microseconds acc_t1, acc_t2, acc_t3, acc_t4, acc_t5,\
    acc_conver_q_over_5, acc_conver_bgmw, diff, min_t12; // time accumulation
    ...
    for( int idx = 1; idx <= TEST_NUM; ++idx){
        ...
        if(TEST_PIPPENGER_Q_OVER_5_2_c){
            /*FKSX method for q = 2^c */
            st = std::chrono::steady_clock::now();
            for(size_t i = 0; i< LOOP_NUM; ++i)
            {
                ret_P_affine_5 = pippenger_variant_q_over_5_2_c(SCALARS_ARRAY);
            }
            ed = std::chrono::steady_clock::now();   
            diff = std::chrono::duration_cast<std::chrono::microseconds>(ed - st);
            acc_t5 += diff;
    ...
    if(TEST_PIPPENGER_Q_OVER_5_2_c){
        std::cout << "\n4. FKSX 'nh+ q/5'. Wall clock time elapse is: " << std::dec << acc_t5.count()/(TEST_NUM*LOOP_NUM) << " us "<< std::endl;
        std::cout << ret_P_affine_5.x <<std::endl;
        std::cout << ret_P_affine_5.y <<std::endl;
        std::cout << std::endl;
    }

    std::cout << "5. pippenger_blst_built_in. Wall clock time elapse is: " << std::dec << acc_t4.count()/(TEST_NUM*LOOP_NUM) << " us "<< std::endl;
    ...
    if(TEST_PIPPENGER_Q_OVER_5_2_c){
    std::cout << "Improvement, blst FKSX_q_over_5 vs pipp: " <<\
    std::fixed << std::setprecision(3) << 100*float(acc_t4.count() - acc_t5.count())/float(acc_t4.count()) << '%' <<std::endl;
    }
...
int main(){
    ...
    if(TEST_PIPPENGER_Q_OVER_5_2_c) init_pippenger_q_over_5_2_c();
    
    // Test should be done between init_xx() and free_init_xx();
    test_pippengers();

    if(TEST_PIPPENGER_BGMW95) free_init_pippenger_BGMW95();
    if(TEST_PIPPENGER_Q_OVER_5_CHES) free_init_pippenger_CHES_q_over_5();
    if(TEST_PIPPENGER_Q_OVER_5_2_c) free_init_pippenger_q_over_5_2_c();
